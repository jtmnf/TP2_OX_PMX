...
class Algorithm:
	...
	def run(self):
		...
		self.StatStdDevBestByRun = [np.sqrt(sum([(best_val - self.StatAvgBestByRun[generation]) ** 2 for best_val in GenerStats]) / len(GenerStats)) for generation, GenerStats in enumerate(StatGener)]
